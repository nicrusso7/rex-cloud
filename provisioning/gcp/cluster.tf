...
module "gke_service_account" {
  ...
  name        = "rex-gym-service-account"
  project     = var.project
  description = "The Service Account for the Rex Gym cluster."
}
...
module "vpc_network" {
  ...
  name_prefix = "${module.gke_cluster.name}-network-${random_string.suffix.result}"
  project     = var.project
  region      = var.region
...
module "tiller" {
  source = "github.com/gruntwork-io/terraform-kubernetes-helm.git//modules/k8s-tiller?ref=v0.6.1"
...
locals {
  tiller_namespace = "tiller-ops"

  # setup Tiller to manage the default Namespace.
  resource_namespace = "default"

  # Install an older version of Tiller to match the Helm library version used in the Terraform helm provider.
  tiller_version = "v2.11.0"

  # Store the CA Secret in the kube-system Namespace, given that only cluster admins should access these.
